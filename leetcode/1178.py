...
class Solution:
    def findNumOfValidWords(self, words, puzzles):
        len_words, len_puzzles = len(words), len(puzzles)
        res = []
        for i in range(len_puzzles):
            cur = 0
            set_puzzle = set(puzzles[i])
            for j in range(len_words):
                dict_word = Counter(words[j])
                if puzzles[i][0] in dict_word:
                    flag = True
                    for key in dict_word:
                        if key not in set_puzzle:
                            flag = False
                            break
                    if flag:
                        cur += 1
